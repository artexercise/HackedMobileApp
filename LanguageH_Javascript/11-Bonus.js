Reddit_LearnProgramming = "https://www.reddit.com/r/learnprogramming/wiki/faq";
Martyr2_ProjectList = "http://www.dreamincode.net/forums/topic/78802-martyr2s-mega-project-ideas-list/";
...
ProjectList = [TextProject, NetworkingProject, ClassesProject, ThreadingProject, WebProject, FilesProject, DatabasesProject, GraphicsAndMultimediaProject, GamesProject];
ProjectTitleList = ["Text", "Networking", "Classes", "Threading", "Web", "Files", "Databases", "Graphics And Multimedia", "Games"];

function listProjects()
{
	for(i=0;i<ProjectList.length;i++)
	{
		d.writeln("<h2>",ProjectTitleList[i],"</h2>");
		listProjectIdeas(i);
	}
}

function listProjectIdeas(i)
{
	d.writeln("<ul>");
	for(j=0;j<ProjectList[i].length;j++)
	{
		listitem = ProjectList[i][j];
		listitemSep = " – ";
		listitemSeptIndex = listitem.indexOf(listitemSep);
		listitemTitle = listitem.substring(0,listitemSeptIndex);
		listitemDescription = listitem.substring(listitemSeptIndex + 3);

		d.writeln("<li class='listitemTitle'>",listitemTitle,"</li>");
		d.writeln("<ul class='listitemDescription'>",listitemDescription,"</ul>");
	}
	d.writeln("</ul>");
}